...
namespace DapperEstate.Areas.Admin.Service
{
    public class CityService : ICityService
    {
        ...
        public Task UpdateCityAsync(UpdateCityDto city)
        {
            throw new NotImplementedException();
        }

        public async Task<GetCityDto> GetCityAsync(int id)
        {
            string query = "Select * From TblLocation Where LocationID=@p1";
            var parameter = new DynamicParameters();
            parameter.Add("@p1", id);
            var connection = _context.CreateConnection();
            var values = await connection.QueryFirstOrDefaultAsync<GetCityDto>(query, parameter);
